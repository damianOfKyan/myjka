<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">{{ translate('messages.WashingProcedures.Index.Self') }}</h1>
    <div class="mb-6 flex justify-between items-center">
      ...
      <inertia-link class="btn-indigo" :href="route('washing-procedures.create')">
        <span>{{ translate('messages.Create') }}</span>
        <span class="hidden md:inline">{{ translate('messages.WashingProcedures.Index.Self') }}</span>
      </inertia-link>
    </div>
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        <tr class="text-left font-bold">
          <th class="px-6 pt-6 pb-4">{{ translate('messages.WashingProcedures.Index.Name') }}</th>
          <th class="px-6 pt-6 pb-4">{{ translate('messages.WashingProcedures.Index.Description') }}</th>
        </tr>
        <tr v-for="washingProcedure in washingProcedures.data" :key="washingProcedure.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
        ...
        <tr v-if="washingProcedures.data.length === 0">
          <td class="border-t px-6 py-4" colspan="4">{{ translate('messages.NoWashingProcedures') }}</td>
        </tr>
      </table>
    </div>
    <pagination class="mt-6" :links="washingProcedures.links" />
  </div>
</template>
...
export default {
  metaInfo: { title: 'Washing Procedures' },
  components: {
    Icon,
    Pagination,
    SearchFilter,
  ...
  watch: {
    form: {
      ...
      handler: throttle(function() {
        this.$inertia.get(this.route('washing-procedures'), pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
